...
const RestoreStatus = ({ showSidebar }) => {
    ...
    const restore_status = async (id) => {
        await fetch('http://localhost:5000/restorestatus', {
            method: 'POST',
            headers: {
                                    ...
                                    {filterStatus.map((status, index) => (
                                        <div key={index}>
                                            <div className="flex items-center lg:space-x-6 sm:space-x-4">
                                                <p className={`lg:w-[260px] ${showSidebar ? 'w-[250px]' : 'w-[300px] lg:w-[300px]'}`} style={{marginLeft: '18px'}}>{status.data.name}</p>
                                                <p className={`${showSidebar ? 'w-[410px]' : 'w-[470px]'} truncate`}>{status.data.description}</p>
